...
export function DashboardSidebar() {
  ...
  const searchParams = useSearchParams(); const isDemo = searchParams.get("demo") === "1";
    ...
    <aside className="flex w-full flex-col justify-between border-b border-sand bg-white p-6 lg:h-screen lg:w-72 lg:border-b-0 lg:border-r">
      <div>
        ...
        <nav className="mt-4 space-y-1">
          {links.map((link) => {
          const href = isDemo ? \${link.href}?demo=1` : link.href;and usehref={href}on the<Link>instead ofhref={link.href}`
