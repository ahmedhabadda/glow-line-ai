...
export function ChatWidget({
  ...
  clinicId?: string;
  greeting?: string;
  /** Renders inline filling its parent container instead of a floating
   * bottom-right launcher — use inside a fixed-size preview panel. */
  embedded?: boolean;
} = {}) {
  ...
  async function send() {
    ...
    try {
      const response = await fetch("/api/chat", {
        ...
        body: JSON.stringify({ messages: nextMessages, clinicId, leadId }),
      });
          ...
          {messages.map((message, index) => (
            <div
              ...
              className={`max-w-[90%] rounded-2xl px-3 py-2 ${
                message.role === "assistant" ? "bg-mist text-ink" : "ml-auto bg-sage text-ivory"
              }`}
            >
              {message.content}
            </div>
